    ...
    <div class="list-group">
        ...
        <?php else: ?>
            ...
            foreach ($viagens as $viagem):
                // Extrair a data da viagem
                $dataViagem = $viagem["serviceDate"];
                // Comparar com a data de hoje
                $mostrarBotao = ($dataViagem === $hoje);
            ?>
                    ...
                    <div class="text-center mt-2">
                        <?php if ($mostrarBotao): ?>
                            <button class="btn btn-outline-success btn-sm open-modal"
                                    ...
                                    data-paxadt="<?= $viagem["paxADT"] ?>"
                                    data-paxchd="<?= $viagem["paxCHD"] ?>"
                                    data-flight="<?= htmlspecialchars($viagem["FlightNumber"]) ?>"
                                    data-client="<?= htmlspecialchars($viagem["NomeCliente"]) ?>"
                                    data-clientnumber="<?= htmlspecialchars($viagem["ClientNumber"]) ?>">
                                ℹ️ Detalhes
                            </button>
                        <?php endif; ?>
                    </div>
...
function filterTrips(dateFilter) {
  ...
  console.log("Hoje: ", todayFormatted); // Verifique se a data de hoje está correta

  let filteredTrips = [];
  if (dateFilter === "yesterday") {
    filteredTrips = viagens.filter(viagem => formatDate(new Date(viagem.serviceDate)) === yesterdayFormatted);
  } else if (dateFilter === "today") {
    filteredTrips = viagens.filter(viagem => formatDate(new Date(viagem.serviceDate)) === todayFormatted);
  } else if (dateFilter === "tomorrow") {
    filteredTrips = viagens.filter(viagem => formatDate(new Date(viagem.serviceDate)) === tomorrowFormatted);
  }

  console.log(filteredTrips); // Verifique o array filtrado
    ...
    function updateTripsDisplay(filteredTrips) {
      ...
      if (filteredTrips.length === 0) {
        ...
        filteredTrips.forEach(viagem => {
          ...
          // Definir a cor da borda e do botão com base no tipo de viagem
          let borderColor = "#ffc107"; // Dourado para Shared (serviceType = 0)
let buttonColor = "btn-outline-warning"; // Dourado para Shared

if (viagem.serviceType === "1" || viagem.serviceType === 1) {
  borderColor = "#007bff"; // Azul para Private (serviceType = 1)
  buttonColor = "btn-outline-primary"; // Azul para o botão também em Private
}
...
listGroup.innerHTML += viagemHtml;
    ...
    document.querySelectorAll(".open-modal").forEach(button => {
      button.addEventListener("click", function () {
        ...
        const paxADT = this.getAttribute("data-paxadt");
        const paxCHD = this.getAttribute("data-paxchd");
        const flight = this.getAttribute("data-flight");
        const client = this.getAttribute("data-client");
        const clientNumber = this.getAttribute("data-clientnumber");
        ...
        document.getElementById("modalADT").textContent = paxADT;
        document.getElementById("modalCHD").textContent = paxCHD;
        document.getElementById("modalClient").textContent = client;
        document.getElementById("modalClientNumber").textContent = clientNumber;

        // Caso necessário, armazene o ID da viagem para referência
