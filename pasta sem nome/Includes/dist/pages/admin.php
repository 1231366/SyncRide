...
<script src="path_para_adminlte.js"></script> <!-- Substituir pelo caminho correto -->
...
?>
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['xmlFile']) && $_FILES['xmlFile']['error'] == 0) {
      ...
      $sql = "INSERT INTO Services 
              (serviceDate, serviceStartTime, paxADT, paxCHD, serviceStartPoint, serviceTargetPoint, FlightNumber, NomeCliente, ClientNumber)
              VALUES 
              (:serviceDate, :serviceStartTime, :paxADT, :paxCHD, :serviceStartPoint, :serviceTargetPoint, :FlightNumber, :NomeCliente, :ClientNumber)";
      ...
      if (isset($xml->Groupings->Grouping)) {
          foreach ($xml->Groupings->Grouping as $service) {
              $serviceData = [
                  ...
                  'FlightNumber' => isset($service->bookings->bookingItem->dropoff->pickupPoint->flightNumber) ? (string) $service->bookings->bookingItem->dropoff->pickupPoint->flightNumber : 'Não disponível',
